...
- В существующих worktree также встречаются `codex/*` (создаются агентами, не создавать вручную через `git checkout -b`), `docs/*` для handoff-документации и `hardening/*`; это рабочие префиксы сверх канонических четырёх.
