...
def check_size_and_offset(profile: FitmentProfile, rim: RimSpec, axle: str) -> RuleResult:
    ...
    if match is None:
        ...
        # Размер близок к approved (±1"), но не подтверждён — проверка на месте.
        return RuleResult(
            rule=rule,
            status=VerdictStatus.compatible_with_conditions,
            reason_code=ReasonCode.non_approved_size_check_required,
            axle=axle,
            detail=detail,
        )
    ...
    if not rim.offset_et_mm.is_known:
        return RuleResult(
            rule=rule,
            status=VerdictStatus.compatible_with_conditions,
            reason_code=ReasonCode.offset_not_verified,
            axle=axle,
