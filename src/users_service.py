...
SUPPORTED_IDENTITY_PROVIDERS: Final[frozenset[str]] = frozenset(
    {
        "telegram",
        "avito",
        "yandex",
...
async def ensure_user_identity(
    ...
    *,
    ...
    """Find or create a canonical user for an external identity.

    The helper owns a nested transaction so it is atomic both when called by a
    legacy endpoint without an outer transaction and when called from an
    existing transaction. The advisory lock serializes concurrent attempts for
    the same provider/subject before either row is created.

    ``username`` is retained only for the Telegram compatibility path. It is
    not a generic profile field and is rejected for future providers.
    """
    provider, provider_subject = _validate_identity(provider, provider_subject)
