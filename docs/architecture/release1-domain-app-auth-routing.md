...
# 16. Связанные способы входа

Email OTP и Telegram могут подтверждать одного человека, но не объединяются по
email, username или совпадению имени автоматически. В `/app/settings` пользователь
явно добавляет второй способ входа, доказывая владение им отдельным действующим
Telegram proof или Email OTP. Эта проверка никогда не меняет текущую сессию.

Если подтверждённый способ уже принадлежит другому каноническому аккаунту,
backend выдаёт короткоживущий одноразовый merge token. Только после понятного
подтверждения пользователя выполняется одна транзакция: выбирается старейший
`users.created_at` (при равенстве — меньший `users.id`), связанные данные и
identity переносятся к нему, а исходный пользователь архивируется ссылкой
`merged_into_user_id`. `users` не удаляются.

Перенос охватывает задачи, assets, черновики и fitment-данные, платежи, ledger,
credit packages, analytics и identities. `render_feedback` следует за задачей
через составной foreign key. Текущий баланс — только кэш суммы действующих
пакетов: после переноса он пересчитывается; выдача стартового пакета не
повторяется.

Ни один клиентский запрос не содержит `users.id`, списка записей или целевого
аккаунта. В интерфейсе показываются лишь безопасные значения: маскированный
email и `@username` Telegram. Отвязка и замена способов входа не входят в
Release 1.
