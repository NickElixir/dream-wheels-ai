...
**READY FOR PR1 — VNEXT FOUNDATION.** Use current `staging` as the runtime baseline. The documentation preflight is complete and the exact frozen VNext authority is available on `docs/vnext-frozen-authority-sync`. The domain/API contracts are sufficiently identified to start foundation work. PR1 must not change Fitment semantics, credits/payment rules, render lifecycle, auth/session behavior, identity persistence, or request/response contracts.