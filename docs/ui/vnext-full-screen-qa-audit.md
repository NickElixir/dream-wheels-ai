...
База интеграции — только closing-QA PASS V3 из `docs/design-code-vnext-v0.1`, commit `7d6d66b`. Старый V4 использован исключительно как источник трёх новых экранов и переходов; он не стал базой итогового HTML. В [прототипе](../references/application-vnext-state-coverage-prototype-v3.html) добавлены Support, Photo Guide, Documents, desktop-навигация и mobile `Помощь → Support`; из Support есть явный переход в Documents. Экран `Ещё` не возвращён. На момент closing reconciliation Photo Guide использовал фотографии `photo-guide-car.jpg` и `photo-guide-car-bad.jpg` из `webapp/assets/`.
...
**Итог: BLOCKED для `VNEXT_UI_CONTRACT = FROZEN`.** Система уже согласована по базовой композиции: desktop и 390 px не имеют горизонтального overflow, ключевые реальные изображения загружаются, результат и Fitment сохраняют различие визуальной примерки и технической совместимости, основные review-состояния открываются. Но текущий прототип не позволяет честно утвердить типографику и все Fitment-сценарии; мобильные History и touch targets требуют коррекции.
...
## Non-blocking polish / runtime implementation phase
