...
- `0003_storage_buckets_rls.sql` — buckets/policies для Storage
- `0004_preorders_robokassa.sql` — предоплаты через Robokassa
- `0005_credit_topups_ledger.sql` — гибкие Robokassa top-up платежи и `credit_ledger`
- `0006_nullable_preorder_email.sql` — email необязателен для Telegram top-up платежей
- `0010_credit_ledger_idempotency_backfill.sql` — backfill и жёсткий invariant для `credit_ledger.idempotency_key`
