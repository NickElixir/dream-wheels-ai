...
TARGET_MIGRATION = ROOT / "migrations" / "0031_auth_identities.sql"
...
async def _with_temp_schema(callback) -> None:
    ...
    schema = f"test_auth_identities_{uuid.uuid4().hex[:8]}"
    try:
...
def test_auth_identities_migration_contract_is_idempotent_and_backend_only() -> None:
...
class _FakeConn:
    ...
    async def fetchval(self, query: str, *args):
        ...
        if normalized.startswith("INSERT INTO user_identities"):
            key = (args[1], args[2])
            if self.race_identity_conflict:
                return None
...
def test_user_service_supports_telegram_legacy_and_generic_identities() -> None:
    async def run() -> None:
        conn = _FakeConn()
        ...
        avito_user_id = await ensure_user_identity(
            conn,
            provider="avito",
            provider_subject="avito-subject-123",
        )
        assert conn.users[avito_user_id]["telegram_user_id"] is None
        assert conn.identities[("avito", "avito-subject-123")] == avito_user_id
...
def test_auth_identities_migration_backfills_and_preserves_users() -> None:
...
def test_auth_identities_creation_is_race_safe_when_database_is_available() -> None:
        ...
        try:
            ...
            results = await asyncio.gather(
                ensure_user_identity(
                    conn,
                    provider="avito",
                    provider_subject="parallel-subject",
                ),
                ensure_user_identity(
                    contender,
                    provider="avito",
            ...
            assert (
                await conn.fetchval("SELECT count(*) FROM user_identities WHERE provider = 'avito'")
                == 1
            )
        finally:
